...
class ThermalEnvironmentSupervisionTerminal(IntcodeComputer):
    ...
    def _run_program(self):
        while True:
            ...
            parameters = self.program[
                self._pointer + 1 : self._pointer
                + 1
                + self.OPCODES_TO_AMOUNT_OF_PARAMS[opcode]
            ]
    ...
    def _get_opcode_from_instruction(instruction: str) -> int:
        opcode_string = instruction[
            -2:
        ]  # The opcode is based on the last two digits of the instruction.
    ...
    def _pad_modes(cls, instruction: str, opcode: int) -> list[int]:
        ...
        modes = reversed(
            modes
        )  # Modes are read from right to left, but indices start on the left.
