...
def steps_to_after(couples):  # List[Tuple[str, str]]): -> Dict[str: List[str]]:
    edges = {
        node: []
        for node in {couple[0] for couple in couples}
        | {couple[1] for couple in couples}
    }
...
def steps_to_their_dependencies(
    couples,
...
def topological_sorting(couples):  # Dict[str: List[str]]): -> List[str]:
    ...
    no_incoming_edges = {
        node for node in steps_to_dependencies if not steps_to_dependencies[node]
    }
    possible_steps = []
    first_time = True
